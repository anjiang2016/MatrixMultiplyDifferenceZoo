...
def load_mnist_single(pkl_path='/Users/zhaomingming/data_sets/mnist/mnist.pkl'):
    ...
    # 缩放到 32x32（双线性插值）
    # 缩放到 32x32
    X_resized = bilinear_resize(X, (32, 32))
    plt.imsave('mnist32.png',X_resized,cmap='gray',vmin=0,vmax=1)

betas = linear_beta_schedule(100)   # 你的 betas
weights = load_best_model('best_model.npz')

# 采样并保存中间图像
x_final = sample_diffusion_with_save(weights, betas, n_samples=1, save_interval=10, save_dir='sampling_steps')
load_mnist_single()
