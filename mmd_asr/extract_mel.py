...
# ===================== 配置 =====================
AUDIO_DIR = "../mmd_tts/my_voice"
METADATA_FILE = "../mmd_tts/metadata.csv"
...
# ===================== 静音裁剪函数 =====================
def trim_silence_from_mel_(mel, threshold=ENERGY_THRESHOLD):
    """
...
def trim_silence_from_mel(mel, threshold=ENERGY_THRESHOLD, keep_edges=0.2, hop_length=HOP_LENGTH, sr=SAMPLE_RATE):
    frame_energy = mel.mean(axis=1)
    valid = np.where(frame_energy > threshold)[0]
    if len(valid) == 0:
        return mel[:1, :]
    start = max(0, valid[0] - int(keep_edges * sr / hop_length))
    end = min(mel.shape[0], valid[-1] + 1 + int(keep_edges * sr / hop_length))
    return mel[start:end, :]
# ===================== 加载元数据 =====================
...
def main():
    ...
    for fname, text in metadata:
        ...
        # ---- 裁剪静音 ----
        mel_trimmed = trim_silence_from_mel(mel_raw, threshold=ENERGY_THRESHOLD,keep_edges=0.35, hop_length=HOP_LENGTH, sr=SAMPLE_RATE)
