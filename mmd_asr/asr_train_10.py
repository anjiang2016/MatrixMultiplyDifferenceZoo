...
def prepare_asr_batch(sample):
    ...
    # 在目标序列前后插入空白（0）
    #text_ids = np.concatenate([[0], text_idsi_, [0]])
...
def init_asr_params(vocab_size, mel_dim=80, embed_dim=256, num_layers=4, num_heads=8, max_seq_len=200):
    ...
    return params
def decode_ctc(logits, blank=0, merge_repeats=True):
    """
    CTC 贪心解码
    logits: (T, vocab) 或 (batch, T, vocab)
    """
    if logits.ndim == 3:
        logits = logits[0]  # 取第一个 batch
    preds = np.argmax(logits, axis=-1)  # (T,)
    decoded = []
    prev = None
    for p in preds:
        if p != blank and (p != prev or not merge_repeats):
            decoded.append(p)
        prev = p
    return decoded
def test_asr(params, mel, text_ids, idx2char, blank=0):
    ...
    # 解码（取第一个 batch）
    decoded_ids = decode_ctc(logits, blank=blank)
...
# ========== 训练循环 ==========
def train_asr():
    samples, char2idx, idx2char, vocab_size = load_asr_data('../mmd_tts/tts_data.npz')
    # ---- 检查词汇表 ----
    print("=== 词汇表检查 ===")
    ...
    params = init_asr_params(vocab_size, mel_dim=80, embed_dim=768, num_layers=8, num_heads=8, max_seq_len=mel.shape[1])
    ...
    for epoch in range(epochs):
        lr = warmup_cosine_decay(epoch, warmup_epochs=200, total_epochs=epochs, lr_init=lr_init, lr_min=1e-8)
        #lr = cosine_decay(epoch, total_epochs=epochs, lr_init=lr_init, lr_min=1e-8)
        for i in range(10):
            mel, text_ids, input_length, target_length = prepare_asr_batch(samples[i])
            mel = normalize_mel(mel)
            ...
            if i==0:
                grads_accum = {k: v.copy() for k,v in grads.items()}
            else:
        ...
        grads_accum = clip_grad_norm(grads_accum, max_norm=1.0)
        params, step = adam_update(params, grads_accum, lr, step,beta1=0.9,beta2=0.99)
        #for key in params:
        #    if key in grads:
        ...
        if (epoch+1) % 10 == 0:
            ...
            if loss < best_loss:
                best_loss = loss
                np.savez('asr_best_10.npz', **params)
