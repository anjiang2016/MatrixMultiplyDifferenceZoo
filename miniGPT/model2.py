...
def forward_transformer(params, input_ids, mask=None):
    ...
    mask: (batch, seq_len) 或 (batch, 1, seq_len, seq_len) 可选
    """
    ...
    for l in range(num_layers):
        ...
        attn, dropout_mask = dropout(attn, dropout_rate)
        ...
        ff_h = relu(ff_h)  # relu 不返回 cache
        ff_h, ff_dropout_mask = dropout(ff_h, dropout_rate)
        ...
        caches[f'layer_{l}'] = {
            ...
            'ff_dropout_mask': ff_dropout_mask,
...
def backward_transformer(dlogits, caches, params):
    ...
    # ---- 输出层反向 ----
    dx, dw_out, db_out = d_linear(dlogits, caches['x_final'], params['output_w'], params['output_b'])
    ...
    for l in range(num_layers - 1, -1, -1):
        ...
        scores = layer['scores']
        attn = layer['attn']
        dropout_mask = layer['dropout_mask']
        ...
        ff_dropout_mask = layer['ff_dropout_mask']
        ...
        # ---- FF 反向 ----
        # 前馈第二层线性
        dh, dw_ff2, db_ff2 = d_linear(dx, ff_linear2_cache['x'], layer['ff_w2'], layer['ff_b2'])
        grads[f'layer_{l}_ff_w2'] = dw_ff2
        grads[f'layer_{l}_ff_b2'] = db_ff2
        # dropout 反向
        dh, _ = d_dropout(dh, ff_dropout_mask)
        # relu 反向 (relu 不返回 cache，直接使用前向输出)
        dh = d_relu(dh, layer['ff_h'])  # 需要保存 ff_h，我们未保存，需在 forward 中添加
        # 第一层线性反向
        dx_ff, dw_ff1, db_ff1 = d_linear(dh, ff_linear1_cache['x'], layer['ff_w1'], layer['ff_b1'])
        grads[f'layer_{l}_ff_w1'] = dw_ff1
        grads[f'layer_{l}_ff_b1'] = db_ff1

        # 残差 x = x_ln1 + ff_out，所以 dx 还需加上 FF 的梯度
        dx = dx_ff + dx  # 注意：dx 是来自上一层（输出端）的梯度
        ...
        # ---- 多头注意力反向 ----
        # 注意力输出线性层反向
        dx_attn, dw_o, db_o = d_linear(dx, attn_linear_cache['x'], layer['w_o'], layer['b_o'])
        grads[f'layer_{l}_w_o'] = dw_o
        grads[f'layer_{l}_b_o'] = db_o

        # 恢复形状 (batch, seq, embed) -> (batch, heads, seq, head_dim)
        dx_attn = dx_attn.reshape(layer['x_in'].shape[0], layer['x_in'].shape[1], num_heads, head_dim).transpose(0, 2, 1, 3)

        # V 的梯度
        # 由于 dx_attn 是 attn @ V 的梯度，需要分别求 attn 和 V 的梯度
        # dV = attn^T @ dx_attn
        ...
        # dropout 反向
        dattn, _ = d_dropout(dattn, dropout_mask)
        # softmax 反向
        dscores, _ = d_softmax(dattn, softmax_cache)
        ...
        # Q, K, V 的线性层反向
        dx_q, dw_q, db_q = d_linear(dQ, x_in, layer['w_q'], layer['b_q'])
        dx_k, dw_k, db_k = d_linear(dK, x_in, layer['w_k'], layer['b_k'])
        dx_v, dw_v, db_v = d_linear(dV, x_in, layer['w_v'], layer['b_v'])
        ...
        # 残差：x = x_in + attn_out，所以 dx 还需加上 attention 的梯度
        dx = dx_q + dx_k + dx_v + dx  # 注意：dx 现在是来自上层（LN1）的梯度

        # 保存当前 dx 给下一层（更浅层）使用
        # 注意：dx 此时已经是该层输入 x_in 的梯度

    # ---- 词嵌入反向 ----
    # dx 是最后一层输入（即第一层输入 x）的梯度
    # 词嵌入：x = linear(one_hot, embed_weight, bias=0)
