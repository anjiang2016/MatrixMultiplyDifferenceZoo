...
from map_utils import MAP_CONFIG,utm_to_map_pixel,build_transform_matrix
# ========== 1. 配置 ==========
DATAROOT = Path("/Users/zhaomingming/data_sets/v1.0-mini")
OUTPUT_DIR = Path("./map_whole_output")
...
# 加载地图
cfg = MAP_CONFIG[city]
map_img = Image.open(DATAROOT / MAP_CONFIG[city]['image_path'])
map_arr = np.array(map_img)
# 如果是灰度图（2D），扩展为三通道
if map_arr.ndim == 2:
    map_arr = np.stack([map_arr] * 3, axis=2)
...
for idx, entry in enumerate(tqdm(cam_front_items)):
    ...
    OFFSET_X=0.0
    OFFSET_Y=0.0
    T = build_transform_matrix(city, offset_x=OFFSET_X, offset_y=OFFSET_Y)

    # 在每帧中，构造齐次坐标 (3, N)
    P_world_hom = np.vstack([X_world, Y_world, np.ones_like(X_world)])

    # 应用矩阵乘法得到像素坐标
    pixel_hom = T @ P_world_hom  # (3, N)
    X_map = pixel_hom[0, :]
    Y_map = pixel_hom[1, :]
    ...
    col, row, utm_x, utm_y = utm_to_map_pixel([X_map,Y_map], city)
    # 注意：这里 colors 只对应有效像素（mask 为 True 的点），而 col, row 是所有 BEV 点
    # 因此需要将 col, row 也对应到 mask 上
    col_masked = col[mask]
    row_masked = row[mask]
    # 再次过滤地图边界
    valid_mask_map = (row_masked >= 0) & (row_masked < map_arr.shape[0]) & (col_masked >= 0) & (col_masked < map_arr.shape[1])
    if not np.any(valid_mask_map):
        continue
    row_final = row_masked[valid_mask_map]
    col_final = col_masked[valid_mask_map]
    colors_final = colors[valid_mask_map]
    map_arr[row_final, col_final] = colors_final
    # 在 for entry in tqdm(...) 循环内部，累加完成后添加：
    if idx % SAVE_INTERVAL == 0:
        # 保存中间结果
        intermediate_path = city_dir/f"map_step_{idx:06d}.jpg"
        Image.fromarray(map_arr[7633:9633,3296:5296]).save(intermediate_path)
...
Image.fromarray(map_arr).save(OUTPUT_MAP)
