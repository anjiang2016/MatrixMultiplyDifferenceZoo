...
from generate_heatmap import generate_inline_html
# ============================
# 配置
# ============================
...
def generate_heatmap_gt(anns, ego_pose_mat, bev_size, class_mapping, instance_to_category, cat_name_map, sigma=3.0):
    ...
    for ann in anns:
        ...
        x, y = pos_ego[1], pos_ego[0]  # 交换（与BEV映射对齐）
...
def get_sample_from_st(sample_token,geom_indices,instance_to_category,cat_name_map):
    # 确定特征图尺寸（由模型决定）
...
def get_samples(sts,geom_indices_cache,instance_to_category):
    # 加载所有必要的 JSON
    instances = load_json('v1.0-mini/instance.json')
    categories = load_json('v1.0-mini/category.json')
    ...
    for st in tqdm(sts,desc="get gt and image list"):
        ...
        sm=get_sample_from_st(st,geom_indices_cache[st],instance_to_category,cat_name_map)
        sms.append(sm)
    return sms
...
def main():
    ...
    samples = get_samples(sample_tokens,geom_indices_cache,instance_to_category)
