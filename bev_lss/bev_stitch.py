...
if __name__ == "__main__":
    ...
    for idx, (img, calib) in enumerate(zip(img_list, calib_list)):
    ...
    ax.imshow(bev)
    ax.set_title("BEV Panorama (Range ±50m)")
    ax.text(BEV_SIZE//2, 20, '⬆ Front', color='red', weight='bold', ha='center')
    ax.text(BEV_SIZE//2, BEV_SIZE-20, '⬇ Back', color='red', weight='bold', ha='center')
    ax.text(20, BEV_SIZE//2, '⬅ Left', color='red', weight='bold', va='center')
    ax.text(BEV_SIZE-20, BEV_SIZE//2, '➡ Right', color='red', weight='bold', va='center')
    ax.axis('off')
    plt.show()
